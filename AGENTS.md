...
政府統計（まずは総務省「消費者物価指数」）を使い、食料の値上がりを
**上昇率ではなく寄与度（家計に占める重みを掛けた影響）で**見せる静的サイト。
...
3. **検算を消さない。** `scripts/build_cpi.py` は、計算した食料の前年同月比と中分類寄与度の合計が
   公表値と合わなければ停止する。止まったら、チェックを緩めるのではなく原因を調べる。
4. **近似値は近似と表示する。** 生鮮魚介・生鮮野菜・生鮮果物は月別ウエイトのため、品目別寄与度は近似。
...
```bash
python -X utf8 scripts/build_cpi.py        # データ再生成＋検算
